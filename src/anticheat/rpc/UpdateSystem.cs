...
namespace MalumMenu.anticheat.rpc
{
	internal class UpdateSystem : RpcCheck
	{
		...
		private static void ValidateSwitchSystem(PlayerControl player, MessageReader reader, ref bool blockRpc)
		{
			...
			{
				MalumMenu.Log.LogInfo($"Blocked switch update from {player.Data.PlayerName} as lights are not currently sabotaged");
				blockRpc = true;
			}
			...
			{
				MalumMenu.Log.LogInfo($"Blocked switch update from {player.Data.PlayerName} as there is a currently active meeting");
				blockRpc = true;
			}
		}
