    ...
    internal class Sabotage
    {
		...
		*/
        ...
        public static void FixSabotage(SystemTypes system)
        {
            switch (system)
            {
                ...
                case SystemTypes.Electrical:
                    ...
                    // MalumMenu.Log.LogMessage($"Actual: {switches.ActualSwitches}, expected: {switches.ExpectedSwitches}");
                    int amount = switches.ActualSwitches ^ switches.ExpectedSwitches;

                    if (amount == 0)
                    {
                        MalumMenu.Log.LogInfo($"Attempted to fix lights, XOR operation is 0 so that means we have nothing to fix");
                        break;
                    }
                ...
                case SystemTypes.MushroomMixupSabotage:
                    ...
                    {
                        MalumMenu.Log.LogInfo("Attempted to fix Mushroom Mixup, we are not the host so nothing can be done");
                        break;
                    }
                    ...
                    {
                        MalumMenu.Log.LogInfo("Attempted to fix Mushroom Mixup, the sabotage is not enabled so we have nothing to fix");
                        break;
                    }

                    MalumMenu.Log.LogInfo("Attempted to fix Mushroom Mixup, we are the host so it can be fixed");
        ...
        public static bool IsSabotageActive(SystemTypes system)
        {
            ...
            {
                MalumMenu.Log.LogError($"All sabotage types should extend from IActivatable, but yet {system} doesn't");
                return false;
            }
